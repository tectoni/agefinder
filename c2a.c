...
/*******************************************> Calculate Thorium Star <*/
double CalcThStar( double Th, double U, double t )
{
double T_star, Mu, Mth;
t=t*1e6;
if (Th > 90.0) {
	Th = Th * 1.1370904e-4;	// Conversion factors from Th(ppm) to ThO2(wt%)
	U = U *   1.134432e-4;
	T_star = (Th+U*264/270/(exp(4.9475e-11*t)-1)*((exp(9.8485e-10*t)+138*exp(1.55125e-10*t))/139-1)) * 0.878809e4;
}
else
	T_star = Th+U*264/270/(exp(4.9475e-11*t)-1)*((exp(9.8485e-10*t)+138*exp(1.55125e-10*t))/139-1);
return T_star;
}
